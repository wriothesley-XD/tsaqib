        ...
        @if($doc->videoPreviewUrl())
            ...
            <a href="{{ $doc->videoWatchUrl() }}" target="_blank" rel="noopener"
               class="inline-flex items-center gap-1.5 text-xs font-bold text-[var(--gold)] hover:underline -mt-4 mb-8">
                Video tidak dapat dimuat? Buka di {{ $doc->youtubeId() ? 'YouTube' : 'Google Drive' }} <i class="fa-solid fa-arrow-up-right-from-square text-[10px]"></i>
            </a>
        @elseif($doc->isLocalVideo())
